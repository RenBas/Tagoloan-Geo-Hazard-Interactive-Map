...
geojson_data = {
  ...
  "features": [
    {
      "type": "Feature",
      "properties": {"name": "Tagoloan River Basin", "type": "basin_boundary", "area_km2": 1704, "outlet": "Macajalar Bay"},
    ...
    {
      "type": "Feature",
      "properties": {"name": "Flood-prone area — main river corridor", "type": "flood_prone", "hazard_level": "high", "description": "High flood susceptibility along the primary Tagoloan River channel."},
    ...
    {
      "type": "Feature",
      "properties": {"name": "Flood-prone area — east tributary confluence", "type": "flood_prone", "hazard_level": "high", "description": "Localised flood zone along east tributary near Impasug-Ong."},
      "geometry": {"type": "Polygon", "coordinates": [[[124.820,8.508],[124.832,8.500],[124.840,8.488],[124.835,8.472],[124.822,8.465],[124.808,8.470],[124.802,8.482],[124.808,8.498],[124.820,8.508]]]}
    },
    {
      "type": "Feature",
      "properties": {"name": "Flood-prone area — lower east sub-basin", "type": "flood_prone", "hazard_level": "high", "description": "Flood-prone lowland confluence zone near Malaybalay approach."},
      "geometry": {"type": "Polygon", "coordinates": [[[124.828,8.298],[124.842,8.288],[124.850,8.275],[124.845,8.260],[124.832,8.252],[124.818,8.255],[124.810,8.265],[124.815,8.280],[124.828,8.298]]]}
    },
    {
      "type": "Feature",
      "properties": {"name": "Flood-prone area — coastal delta and estuary", "type": "flood_prone", "hazard_level": "very_high", "description": "Estuarine and coastal floodplain. Highest risk zone — storm surge combined with river flooding."},
      "geometry": {"type": "Polygon", "coordinates": [[[124.670,8.215],[124.685,8.210],[124.700,8.208],[124.715,8.210],[124.720,8.205],[124.712,8.198],[124.700,8.195],[124.685,8.195],[124.672,8.198],[124.665,8.208],[124.670,8.215]]]}
    },
    {
      "type": "Feature",
      "properties": {"name": "Landslide-prone area — basin interior", "type": "landslide_prone", "hazard_level": "high", "description": "High susceptibility to landslides across the basin interior. Steep terrain and heavy rainfall."},
      "geometry": {"type": "Polygon", "coordinates": [[[124.698,8.608],[124.718,8.612],[124.738,8.608],[124.758,8.602],[124.778,8.595],[124.795,8.585],[124.808,8.570],[124.815,8.550],[124.820,8.528],[124.822,8.505],[124.820,8.480],[124.815,8.455],[124.812,8.428],[124.814,8.400],[124.818,8.372],[124.820,8.345],[124.818,8.318],[124.812,8.292],[124.805,8.268],[124.795,8.248],[124.782,8.232],[124.768,8.220],[124.752,8.212],[124.735,8.208],[124.718,8.208],[124.702,8.212],[124.688,8.220],[124.675,8.232],[124.665,8.248],[124.658,8.265],[124.655,8.285],[124.654,8.308],[124.655,8.332],[124.658,8.355],[124.662,8.378],[124.668,8.400],[124.674,8.422],[124.678,8.445],[124.680,8.468],[124.679,8.490],[124.676,8.512],[124.672,8.532],[124.670,8.552],[124.672,8.572],[124.680,8.590],[124.690,8.604],[124.698,8.608]]]}
    },
...
# 3. ADD POLYGONS (Hazards & Boundaries)
for feature in geojson_data["features"]:
    ...
    if geom["type"] == "Polygon":
        ...
        # Color coding based on hazard type and level
        if props["type"] == "basin_boundary":
            color, opacity = "rgba(0, 0, 0, 0)", 0
            line_color, line_width = "#333333", 3
        elif props["type"] == "flood_prone":
            if props["hazard_level"] == "very_high":
                color, opacity = "rgba(220, 20, 60, 0.5)", 0.5 # Crimson
            else:
                color, opacity = "rgba(255, 165, 0, 0.4)", 0.4 # Orange
            line_color, line_width = "#8B0000", 1
        elif props["type"] == "landslide_prone":
            color, opacity = "rgba(139, 69, 19, 0.4)", 0.4 # SaddleBrown
            line_color, line_width = "#8B4513", 1
        ...
        fig.add_trace(go.Scattermapbox(
            ...
            name=props["name"],
            hovertemplate=f"<b>{props['name']}</b><br>Hazard Level: {props.get('hazard_level', 'N/A')}<br>{props.get('description', '')}<extra></extra>"
        ))

# 4. ADD RIVERS (LineStrings)
for feature in geojson_data["features"]:
    if feature["geometry"]["type"] == "LineString":
        ...
        # Thicker line for main stem, thinner for tributaries
    ...
    for feature in geojson_data["features"]:
        if feature["properties"]["type"] == st_type:
            ...
            if "alert" in feature["properties"]:
                text += f"<br>⚠️ Alert Level: {feature['properties']['alert']}<br> Critical Level: {feature['properties']['critical']}"
...
# 6. LAYOUT & 3D SETTINGS
fig.update_layout(
    title={
        'text': "<b>Tagoloan River Basin — PAGASA Interactive Hazard Map</b><br><sup>DOST-PAGASA / DENR-MGB Data Visualization</sup>",
        'y': 0.95, 'x': 0.5, 'xanchor': 'center', 'yanchor': 'top',
        'font': dict(size=20, color='#1a1a1a')
    },
    mapbox=dict(
        style="open-street-map", # Free style, no token required
        bearing=0,
        pitch=45, # 3D Tilt effect
        zoom=9,
        center=dict(lat=8.42, lon=124.75)
    ),
...
# 7. SAVE AND SHOW
output_file = "tagoloan_hazard_map.html"
fig.write_html(output_file)
print(f"✅ Interactive Hazard Map saved as '{output_file}'")
